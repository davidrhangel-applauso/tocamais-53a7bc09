...
export function SetlistMusicSelector({ setlistId, setlistName, artistaId, onUpdate }: SetlistMusicSelectorProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load musicas already in this setlist
      ...
      const ids = new Set(setlistMusicasData?.map(sm => sm.musica_id) || []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedMusicIds.size > 0) {
        const inserts = Array.from(selectedMusicIds).map((musicaId, index) => ({
      ...
      toast.success(`${selectedMusicIds.size} músicas salvas na setlist!`);
      setOpen(false);
      onUpdate();
    } catch (error: any) {
  ...
  const allFilteredSelected = filteredMusicas.length > 0 && filteredMusicas.every(m => selectedMusicIds.has(m.id));
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-lg max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecione as músicas para "{setlistName}"
          </DialogDescription>
        </DialogHeader>
          ...
          <>
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">
                {selectedMusicIds.size} de {musicas.length} selecionadas
              </span>
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={selectAll} disabled={allFilteredSelected}>
                  Selecionar todas
                </Button>
                <Button variant="ghost" size="sm" onClick={deselectAll} disabled={selectedMusicIds.size === 0}>
                  Limpar
                </Button>
              </div>
            </div>
            ...
            <ScrollArea className="flex-1 -mx-6 px-6 max-h-[50vh]">
              <div className="space-y-1">
                {filteredMusicas.map((musica) => (
                  ...
                  >
                    <Checkbox
                      checked={selectedMusicIds.has(musica.id)}
            ...
            <Button onClick={handleSave} disabled={saving} className="w-full">
              {saving ? "Salvando..." : `Salvar (${selectedMusicIds.size} músicas)`}
            </Button>
          </>
        )}
      </DialogContent>
